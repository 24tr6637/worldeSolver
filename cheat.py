...
def create_score_map(words) -> list:
    ...
    for word in words:
        word_score = sum(frequency[letter] for letter in set(list(word)))
...
def find_optimal_word(scores, used_letters) -> str:
    ...
    for entry in scores:
        word = entry[0]
        score = entry[1]
        letter_score = sum(letter in used_letters for letter in word)
...
def run(hard=False):
    ...
    while True:
        ...
        if not hard:
            ...
            working_words = list(
                filter(
                    lambda x: set(x).union(avaliable_letters)
                    == avaliable_letters,
                    working_words,
                )
            )
        ...
        green_letters = {optimal_word[index] for index in green}
        yellow_letters = {optimal_word[index] for index in yellow}
